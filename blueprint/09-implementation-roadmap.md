...
- Giữ ba frontend theo actor: Audience Web, Admin Web và Scanner PWA.
...
| 3 | Event-day operations | Đã có nền tảng đầy đủ | Public-key manifest verification/PWA shell hardening và policy xử lý manifest hết TTL. |
...
- Audience Web, Admin Web, Scanner PWA và Scanner Mobile prototype.
...
- Chọn Scanner PWA là client chính để chấm; Scanner Mobile ghi rõ là prototype/alternative.
...
- IndexedDB cho assignment, manifest, pending queue, result history và local checked-in set.
...
- Backend hiện ký manifest bằng HMAC nhưng browser mới kiểm tra presence/scope/TTL. Hướng hoàn thiện là asymmetric signature và pinned public key để PWA verify độc lập.
- Manifest TTL phải bao phủ ca/sự kiện cộng grace period; sai scope/signature mới là hard failure.
- Application-level IndexedDB encryption và key management là production hardening; demo giảm PII và có cleanup policy.
