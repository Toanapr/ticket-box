...
| Message broker | RabbitMQ | Notification, ticket issuance, payment reconciliation, CSV import, AI jobs, retry/DLQ. | Dễ vận hành hơn Kafka, routing/retry/DLQ hợp workflow nghiệp vụ. | Dùng queue riêng cho job quan trọng; log rõ DLQ/retry reason. |
...
| Authentication/Authorization | NestJS auth + JWT/session + RBAC | Đăng nhập, phân quyền audience/organizer/scanner/admin. | Đủ cho đồ án, ít vận hành hơn Keycloak. | Thiết kế adapter/claim rõ để sau này thay bằng OIDC provider. |
...
| Microservices đầy đủ | Scale độc lập, team ownership rõ, fault isolation tốt hơn. | Distributed transaction, observability, triển khai và versioning phức tạp hơn. | Khi đã có tải lớn, nhiều team, module Inventory/Payment/Check-in cần scale độc lập. |
...
| Chống lock-in | Dùng công nghệ OSS, dễ chuyển môi trường. | Tự tích hợp nhiều mảnh ghép hơn. | Chuẩn hóa qua SQL, Redis, AMQP, S3-compatible API, OpenTelemetry-compatible logs/metrics. |
