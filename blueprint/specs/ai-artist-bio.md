...
- AI Artist Bio Service và PDF extractor.
- AI model provider.
- Concert Service/Public Page hiển thị bio đã publish.

## Luồng chính

1. Organizer mở concert thuộc organization của mình và upload PDF press kit.
2. Backend kiểm tra quyền, loại file, kích thước, malware scan và lưu PDF vào object storage.
...
8. Concert Service lưu bio đã publish cho concert và public page hiển thị nội dung mới.
...
| File không phải PDF, quá lớn hoặc malware scan fail | Reject upload hoặc đánh dấu job failed; không tạo draft publishable. |
...
- Bảo mật: PDF là input không tin cậy; phải kiểm tra file, malware, quyền truy cập và không để prompt injection điều khiển system instruction.
...
- [ ] PDF không hợp lệ, quá lớn hoặc malware scan fail không tạo draft publishable.
