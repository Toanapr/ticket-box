...
- Cache worker xử lý outbox invalidation sau thay đổi public data.
...
7. Public API chỉ trả concert `published`; cache/listing/detail được invalidate hoặc prewarm theo outbox record.
...
| Upload seating map lỗi hoặc object storage lỗi | Giữ trạng thái hiện tại; không publish asset lỗi. |
...
| Cache invalidation lỗi | Event được retry; cache có TTL/stale policy, dữ liệu cuối cùng phải được cập nhật. |
...
- Cache: Thay đổi concert/ticket type public phải ghi outbox record để invalidate listing/detail/read model.
...
- [ ] Cập nhật concert/ticket type public ghi outbox invalidation cache.
