...
2. Trước ca, scanner tải signed manifest theo assignment gồm ticket/guest entry hợp lệ, version, TTL, checksum và revoked list.
3. App xác minh chữ ký manifest và lưu local encrypted storage.
4. Khi quét QR, app xác minh chữ ký QR, concert, zone/gate và trạng thái local checked-in set.
...
7. Khi có mạng, app gửi batch event lên Check-in Service bằng cùng event id.
...
| Manifest sai chữ ký, sai checksum hoặc hết TTL | Dừng offline scan và yêu cầu tải manifest hợp lệ. |
...
- Bảo mật: Scanner token ngắn hạn, bind với device/event/gate hoặc zone; manifest lưu local phải mã hóa.
...
- [ ] Manifest hết TTL hoặc sai chữ ký bị từ chối trước khi scan offline.
