...
| [Tải trọng đột biến khi mở bán](core-design-decisions/sale-traffic-spike.md) | Dùng waiting room/virtual queue để làm phẳng traffic, rate limit ở gateway/edge, queue/backpressure cho request giữ vé, autoscale và pre-warm capacity trước giờ mở bán. |
...
| [Check-in offline](core-design-decisions/offline-check-in.md) | Mobile app lưu local durable log, sync idempotent khi online. Với vé thường, ưu tiên preloaded signed ticket manifest theo cổng/khu. |
...
| [AI Artist Bio](core-design-decisions/ai-artist-bio.md) | Upload PDF vào object storage, xử lý async bằng queue/workflow, extract text, sanitize, gọi AI model, human review nếu cần. |
