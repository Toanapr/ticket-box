...
export function TicketTypeForm({ concertId }: TicketTypeFormProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await apiFetch<TicketType>(`/admin/concerts/${concertId}/ticket-types`, {
        method: "POST",
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-4 md:grid-cols-2">
      ...
      <div className="md:col-span-2">
        ...
        >
          {isSaving ? "Saving..." : "Create ticket type"}
        </button>
      </div>
    </form>
  );
