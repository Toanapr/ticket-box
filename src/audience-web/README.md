...
Public concert list and detail pages also read this backend URL from the Next.js server. They call `GET /concerts` and `GET /concerts/:slug`; legacy UUID detail URLs redirect to the canonical slug URL. Concert reads do not fall back to mock data when configuration or the backend is unavailable. Start and seed `src/backend-api` before opening `/concerts`.
