...
import { CheckIcon, CreditCardIcon, QrIcon, TicketIcon, UsersIcon } from "@/components/icons";
...
const buyingSteps = [
  ...
  {
    title: "Thanh toán qua VNPAY hoặc MOMO",
    body: "Chọn phương thức thanh toán, quét QR hoặc mở app tương ứng. Cần thanh toán đúng số tiền và mã đơn.",
  },
  {
...
const notes = [
  ...
  "Nếu thanh toán thất bại, quay lại trang đơn hàng để thử lại hoặc chọn phương thức khác.",
...
export default function GuidePage(): React.ReactElement {
  return (
    <PageShell>
      <Breadcrumbs items={[{ label: "Concerts", href: "/concerts" }, { label: "Hướng dẫn" }]} />
      ...
      <section className="grid gap-8 lg:grid-cols-[1fr_360px] lg:items-start">
        <div className="rounded-lg border border-ticket-obsidian bg-white p-6 shadow-[6px_6px_0_#0d1118] md:p-8">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">
            Trang này giải thích ngắn gọn luồng mua vé cho khán giả: từ chọn concert, giữ chỗ, thanh toán đến nhận e-ticket QR.
          </p>

          <div className="mt-8 grid gap-4">
            {buyingSteps.map((step, index) => (
              <article key={step.title} className="grid gap-4 rounded-lg border border-black/10 bg-ticket-alabaster p-4 sm:grid-cols-[56px_1fr]">
        ...
        <aside className="grid gap-5">
          <div className="rounded-lg border border-black/10 bg-white p-5">
            ...
            <p className="mt-4 text-sm leading-6 text-slate-600">
              Hiện checkout hỗ trợ VNPAY và MOMO. Mỗi đơn có mã thanh toán riêng, không dùng lại QR của đơn cũ.
            </p>
          </div>
          ...
          <div className="rounded-lg border border-black/10 bg-white p-5">
            ...
            <p className="mt-4 text-sm leading-6 text-slate-600">
              Tài khoản giúp bạn theo dõi vé giữ chỗ, vé đã mua và thông tin cơ bản dùng khi checkout.
            </p>
            <Link href="/user" className="mt-4 flex min-h-11 items-center justify-center rounded bg-ticket-green px-4 text-sm font-black uppercase tracking-wide text-white">
              Xem tài khoản
            </Link>
          </div>
          ...
          <div className="rounded-lg border border-ticket-obsidian bg-ticket-obsidian p-5 text-white">
            <div className="flex items-center gap-3">
              ...
              <h2 className="font-display text-xl font-black">Lưu ý khi vào cổng</h2>
            </div>
