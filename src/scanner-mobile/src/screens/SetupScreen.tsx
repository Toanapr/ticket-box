...
export function SetupScreen() {
  ...
  const [accessToken, setAccessToken] = useState(connectionConfig?.accessToken ?? '');
  ...
  async function handleFetchManifest() {
    ...
    try {
      ...
      const manifestRes = await client.getManifest({ assignmentId: assignment.assignmentId });
  ...
  return (
    <ScrollView className="flex-1 bg-zinc-950 p-5">
      ...
      <View className="bg-zinc-900 rounded-3xl p-6 border border-zinc-800 mb-6 shadow-2xl">
        ...
        <View className="mb-5">
          <Text className="text-zinc-400 text-sm mb-2 font-medium">API Base URL</Text>
        ...
        <View className="mb-5">
          <Text className="text-zinc-400 text-sm mb-2 font-medium">Device ID</Text>
          <View className="flex-row items-center bg-zinc-950 border border-zinc-800 rounded-2xl px-4 py-3">
        ...
        <View className="mb-8">
          <Text className="text-zinc-400 text-sm mb-2 font-medium">Access Token</Text>
          <View className="flex-row items-center bg-zinc-950 border border-zinc-800 rounded-2xl px-4 py-3">
        ...
        >
          {isFetchingAssignment ? (
            ...
            <>
              ...
              <Text className="text-white font-bold text-base ml-2">Connect & Fetch</Text>
            </>
          )}
        </TouchableOpacity>
      </View>
      ...
      {assignment && (
        <View className="bg-zinc-900 rounded-3xl p-6 border border-zinc-800 mb-6 shadow-2xl">
          ...
          <View className="flex-row items-center justify-between bg-zinc-950 p-4 rounded-2xl border border-zinc-800 mb-4">
            <View>
              <Text className="text-zinc-500 text-xs font-bold uppercase mb-1">Gate</Text>
              <Text className="text-white font-black text-2xl">{assignment.gateCode}</Text>
            </View>
            <View className="h-10 w-px bg-zinc-800" />
            <View>
              <Text className="text-zinc-500 text-xs font-bold uppercase mb-1 text-right">Zone</Text>
              <Text className="text-white font-black text-2xl text-right">{assignment.zoneCode}</Text>
            </View>
          </View>

          <Text className="text-zinc-400 text-sm mb-6 text-center">
            Event ID: <Text className="font-bold text-zinc-300">{assignment.eventId}</Text>
          </Text>
          ...
          >
            {isFetchingManifest ? (
              ...
              <>
                ...
                <Text className="text-emerald-500 font-bold text-base ml-2">Download Offline Manifest</Text>
              </>
            )}
          </TouchableOpacity>
        </View>
      ...
      {manifest && (
        <View className="bg-emerald-950/30 border border-emerald-900/50 rounded-3xl p-6 mb-10 flex-row items-center">
          ...
          <View className="flex-1">
            <Text className="text-emerald-400 text-lg font-bold mb-1">System Ready</Text>
